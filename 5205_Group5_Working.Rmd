---
title: '5205 Group Project - working code for anlaysis'
...
tinder <- appler::get_apple_reviews(547702041, country = "us", all_results = TRUE)
hinge <- appler::get_apple_reviews(595287172, country = "us", all_results = TRUE)
bumble <- appler::get_apple_reviews(930441707, country = "us", all_results = TRUE)

#Combine dataframes
  raw_data <- rbind(tinder, hinge, bumble)

#Save as csv file, updated March 3th, 2022
#write.csv(raw_data,"~/Desktop/5205_AAFrameworks&MethodsII_VishalLala/Project/raw_data_303.csv", row.names = FALSE)
  
#Save as csv file, updated April 14th, 2022
#write.csv(raw_data,"~/Desktop/5205_AAFrameworks&MethodsII_VishalLala/Project/raw_data_414.csv", row.names = FALSE)
```

```{r}
#Check NA value
check_na <- is.na(raw_data)
table(check_na)

#Add a column "APP" to indicate app names
tinder$APP <- "Tinder"
hinge$APP <- "Hinge"
bumble$APP <- "Bumble"

data <- rbind(tinder, hinge, bumble)

  #Save as csv file, updated March 3th, 2022
#write.csv(data,"~/Desktop/5205_AAFrameworks&MethodsII_VishalLala/Project/data_303.csv", row.names = FALSE)

#Save as csv file, updated April 14th, 2022
#write.csv(data,"~/Desktop/5205_AAFrameworks&MethodsII_VishalLala/Project/data_414.csv", row.names = FALSE)
```


#Combine & Clean collected data
```{r}
#combine two datasets (due to the 500 review limit for appler)
setwd('~/Desktop/5205_AAFrameworks&MethodsII_VishalLala/Project')
  #data303 = read.csv('data_303.csv')
  #data414 = read.csv('data_414.csv')
#combine two collected datasets
  #data_combined = rbind(data303,data414)
#remove duplicated rows
  #data = data_combined[!duplicated(data_combined), ]
#show duplicated rows: all duplicated rows are from Hinge
  #data_duplicated = data_combined[duplicated(data_combined), ]
#Save data as csv file
#write.csv(data,"~/Desktop/5205_AAFrameworks&MethodsII_VishalLala/Project/data.csv", row.names = FALSE)
...
custom_stopwords = rbind(stop_words, data.frame(word = tm::stopwords(), lexicon = 'tm'), data.frame(word = c(tm::stopwords(), 'app', 'great','like', 'likes', 'good', 'better', 'bad', 'don.t', 'doesn.t', 'can.t', 'can','just', 'will','get','bumble','tinder','hinge','didn.t','that','that.s','won.t','you.re','i.ve','it.s','i.m','there.s','there.re', 'love', 'will', 'best', 'liked','well','super'), lexicon = 'custom'))
